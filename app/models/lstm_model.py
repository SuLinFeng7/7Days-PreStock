...
def create_lstm_model(input_shape):
    ...
    # 第一个双向LSTM层
    x = Bidirectional(
        LSTM(LSTM_PARAMS['hidden_dim'], 
             return_sequences=True,
             dropout=LSTM_PARAMS['dropout'],
             recurrent_dropout=LSTM_PARAMS['dropout']/2)
    )(inputs)
    ...
    # 全连接层
    x = Dense(LSTM_PARAMS['hidden_dim'], activation='relu')(x)
    x = BatchNormalization()(x)
    x = Dropout(LSTM_PARAMS['dropout'])(x)
    
    x = Dense(LSTM_PARAMS['hidden_dim']//2, activation='relu')(x)
    x = BatchNormalization()(x)
    x = Dropout(LSTM_PARAMS['dropout'])(x)
...
def train_lstm_model(X_train, y_train, X_test, y_test, progress_callback=None):
    ...
    class ProgressCallback(tf.keras.callbacks.Callback):
        def on_epoch_end(self, epoch, logs=None):
            if progress_callback:
                progress_callback(epoch + 1, LSTM_PARAMS['epochs'])
    ...
    callbacks = [
        ...
        ReduceLROnPlateau(
            monitor='val_loss',
            factor=LSTM_PARAMS['reduce_lr_factor'],
            patience=LSTM_PARAMS['reduce_lr_patience'],
            min_lr=LSTM_PARAMS['min_lr'],
            verbose=1
        ),
        # 训练进度回调
