...
class TestLSTMOptimization(unittest.TestCase):
    def setUp(self):
        """准备测试数据"""
        try:
            print("\n开始获取股票数据...")
            ...
            # 创建测试输出目录
            self.test_dir = "./test_output/lstm_optimization"
    ...
    def test_optimized_lstm(self):
        ...
        try:
            ...
            # 测试未来预测
            future_predictions = self._test_future_predictions(model)
            
            # 保存预测结果到Excel
            self._save_predictions_to_excel(future_predictions)
    ...
    def _plot_predictions(self, y_pred, y_true):
        ...
        plt.plot(y_true[-50:], label='真实值', color='blue')
        plt.plot(y_pred[-50:], label='预测值', color='red', linestyle='--')
        plt.title('优化后的LSTM模型预测结果对比（最近50天）')
        plt.xlabel('时间步')
        plt.ylabel('股票价格')
        plt.legend()
        plt.grid(True)
        ...
        # 保存图片
        plt.savefig(os.path.join(self.test_dir, 'lstm_predictions_optimized.png'))
        plt.close()
    ...
    def _plot_future_predictions(self, predictions):
        ...
        # 保存图片
        plt.savefig(os.path.join(self.test_dir, 'lstm_future_predictions_optimized.png'))
        plt.close()

    def _save_predictions_to_excel(self, predictions):
        """保存预测结果到Excel文件"""
        try:
            # 创建日期列表
            ...
            df = pd.DataFrame({
                '日期': dates,
                'LSTM预测价格': predictions
            })
            
            # 保存到Excel
            excel_path = os.path.join(self.test_dir, 'lstm_predictions_optimized.xlsx')
